...
def multiclass_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, learning_rate=0.001):
    ...
    conv1 = Conv2D(
        16, kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"
    )(inputs)
    ...
    conv2 = Conv2D(
        32, kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"
    )(pooled1)
    ...
    conv3 = Conv2D(
        64, kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"
    )(pooled2)
    ...
    conv4 = Conv2D(
        128, kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"
    )(pooled3)
    ...
    conv5 = Conv2D(
        256, kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"
    )(pooled4)
    ...
    model.compile(
        optimizer=optimizer, loss="categorical_crossentropy", metrics=["accuracy", mean_iou]
    )
