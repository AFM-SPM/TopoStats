"""Tests for the grainstats module."""
from pathlib import Path

import pandas as pd

from topostats.grainstats import GrainStats

# Specify the absolute and relattive tolerance for floating point comparison
TOLERANCE = {"atol": 1e-07, "rtol": 1e-07}

BASE_DIR = Path.cwd()
RESOURCES = BASE_DIR / "tests" / "resources"


def test_grainstats(minicircle_grainstats: GrainStats, minicircle_grainstats_20220524: pd.DataFrame) -> None:
    """Test the overall GrainStats class."""
    statistics = minicircle_grainstats.calculate_stats()

    pd.testing.assert_frame_equal(statistics["statistics"], minicircle_grainstats_20220524)
