...
def test_threshold_otsu(minicircle_grain_threshold_otsu: Grains) -> None:
    ...
    assert minicircle_grain_threshold_otsu.thresholds["upper"] == 1.674593388380477
...
def test_threshold_stddev(minicircle_grain_threshold_stddev: Grains) -> None:
    ...
    assert minicircle_grain_threshold_stddev.thresholds == {"upper": 0.7723130579879989, "lower": -0.44925918420179745}
...
def test_mask_minicircle(minicircle_grain_mask: Grains, plotting_config: dict, plot_dict: dict, tmp_path) -> None:
    ...
    assert minicircle_grain_mask.directions["upper"]["mask_grains"].sum() == 55866
...
def test_clear_border(minicircle_grain_clear_border: Grains, plotting_config: dict, plot_dict: dict, tmp_path) -> None:
    ...
    assert minicircle_grain_clear_border.directions["upper"]["tidied_border"].sum() == 51659
...
def test_remove_noise(minicircle_grain_remove_noise: Grains, plotting_config: dict, plot_dict: dict, tmp_path) -> None:
    ...
    assert minicircle_grain_remove_noise.directions["upper"]["removed_noise"].sum() == 46591
...
def test_calc_minimum_grain_size_pixels(minicircle_minimum_grain_size) -> None:
    ...
    assert minicircle_minimum_grain_size.minimum_grain_size == 1575.75
...
def test_remove_small_objects(
    ...
    assert minicircle_small_objects_removed.directions["upper"]["removed_small_objects"].sum() == 42482
...
def test_label_regions(
    ...
    assert minicircle_grain_labelled_post_removal.directions["upper"]["labelled_regions_02"].sum() == 485986
...
def test_colour_regions(minicircle_grain_coloured: Grains, plotting_config: dict, plot_dict: dict, tmp_path) -> None:
    ...
    assert minicircle_grain_coloured.directions["upper"]["coloured_regions"].sum() == 61861.088
