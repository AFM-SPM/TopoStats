...
def test_threshold_otsu(minicircle_grain_threshold_otsu: Grains) -> None:
    ...
    assert minicircle_grain_threshold_otsu.thresholds["above"] == pytest.approx(1.6955888564850083)
...
def test_threshold_stddev(minicircle_grain_threshold_stddev: Grains) -> None:
    ...
    assert minicircle_grain_threshold_stddev.thresholds == pytest.approx(
        {"below": -5.934646110767899, "above": 0.7711821893707078}
    )
...
def test_mask_minicircle(minicircle_grain_mask: Grains) -> None:
    ...
    assert minicircle_grain_mask.directions["above"]["mask_grains"].shape == (1024, 1024)
    assert minicircle_grain_mask.directions["above"]["mask_grains"].sum() == 54964
...
def test_clear_border(minicircle_grain_clear_border: Grains) -> None:
    ...
    assert minicircle_grain_clear_border.directions["above"]["tidied_border"].shape == (1024, 1024)
    assert minicircle_grain_clear_border.directions["above"]["tidied_border"].sum() == 50793
...
def test_remove_noise(minicircle_grain_remove_noise: Grains) -> None:
    ...
    assert minicircle_grain_remove_noise.directions["above"]["removed_noise"].shape == (1024, 1024)
    #    assert minicircle_grain_remove_noise.directions["above"]["removed_noise"].sum() == 45813
    assert minicircle_grain_remove_noise.directions["above"]["removed_noise"].sum() == 44065
...
def test_remove_small_objects(minicircle_small_objects_removed: Grains) -> None:
    ...
    assert minicircle_small_objects_removed.directions["above"]["removed_small_objects"].shape == (1024, 1024)
    assert minicircle_small_objects_removed.directions["above"]["removed_small_objects"].sum() == 41960
...
def test_area_thresholding(minicircle_area_thresholding: Grains) -> None:
    ...
    assert minicircle_area_thresholding.directions["above"]["removed_small_objects"].shape == (1024, 1024)
    assert minicircle_area_thresholding.directions["above"]["removed_small_objects"].sum() == 398845
...
def test_label_regions(minicircle_grain_labelled_post_removal: Grains) -> None:
    ...
    assert minicircle_grain_labelled_post_removal.directions["above"]["labelled_regions_02"].shape == (1024, 1024)
    assert minicircle_grain_labelled_post_removal.directions["above"]["labelled_regions_02"].sum() == 479645
...
def test_colour_regions(minicircle_grain_coloured: Grains) -> None:
    ...
    assert minicircle_grain_coloured.directions["above"]["coloured_regions"].shape == (1024, 1024, 3)
    assert minicircle_grain_coloured.directions["above"]["coloured_regions"].sum() == 60991.13700000002
