...
def test_remove_quadratic_unmasked(
    ...
    assert minicircle_initial_quadratic_removal.images["initial_quadratic_removal"].sum() == pytest.approx(-1672538.8578738687)
...
def test_get_threshold_stddev(minicircle_threshold_stddev: np.array) -> None:
    ...
    assert minicircle_threshold_stddev.thresholds == pytest.approx({'lower': -8.283235478318018, 'upper': -0.9269936645505799})
...
def test_remove_quadratic_masked(
    ...
    assert minicircle_masked_quadratic_removal.images["masked_quadratic_removal"].sum() == pytest.approx(169411.34987849486)
