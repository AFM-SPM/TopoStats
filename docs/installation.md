...
<li> If you *are* on an M1/M2 Mac, you will need to to build libasd yourself. This can be done by cloning the `libasd` repository, `pip install cmake`, and run `pip install .` in the `libasd` repository. More info can be found on the [libasd gitHub page](https://github.com/ToruNiina/libasd#building-by-yourself)
