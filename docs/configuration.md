...
| :--------------------------------------------------- | :------------------------------------------------- | :------------------------------------------------- | :-------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base_dir` | | string | `./` | Directory to recursively search for files within. (See [Absolute v Relative Paths](#absolute-v-relative-paths)) |
| `output_dir` | | string | `./output` | Directory that output should be saved to. (See [Absolute v Relative Paths](#absolute-v-relative-paths)) |
...
| `filter` <br> ∟ `remove_scars` | `run` | bool | `true` | Whether to run scar removal. |
...
| | `grain_crop_padding` | int | `1` | The padding to apply to grains during cropping. Must be >= 1. |
...
| `grains` <br> ∟ `unet_config` | `path_to_model` | str | `null` | The path to the U-Net model to override traditional segmentation. Supply a path to a tensorflow U-net model to use, else U-Net segmentation will be skipped. |
...
| | `remove_disconnected_grains` | bool | `False` | Whether to remove subgrains that are not connected to the original grain mask. |
| `grains` <br> ∟ `vetting` | `class_region_number_thresholds` | list[tuple[int, int, int]] | `null` | Class region number thresholds, list of lists, `[[class, low, high]]`, eg: `[[1, 2, 4], [2, 1 ,1]]` for class 1 to have 2-4 regions and class 2 to have 1 region. Can use Noneto not set an upper/lower bound. |
...
| | `class_names` | list(str) | `["DNA", "Protein"]` | Names of the objects found by the segmentation methods (objects are linked to the `class_number` output in the grainstats csv). |
| `disordered_tracing` | `run` | boolean | `true` | Whether to run the Disordered Traces pipeline. Options : true, false |
| | `class_index` | int | `1` | The class index to run tracing on. 1 is typically the DNA class. |
...
| `disordered_tracing` <br> ∟ `mask_smoothing_params` | `gaussian_sigma` | float | `2` | Amount of smoothing by a gaussian kernel. This will compete with `dilation_iteration` to see which changes the grain mask least, ensuring quality over different scan sizes. |
| | `dilation_iterations` | int | `2` | The the number of dilations to perform to smooth. This will compete with `gaussian_sigma` to see which changes the grain mask least, ensuring quality over different scan sizes. |
| | `holearea_min_max` | list | `[0, null]` | As smoothing fill holes in the mask, this replaces those within a size range (in nm^2). |
| `disordered_tracing` <br> ∟ `skeletonisation_params` | `method` | str | `topostats` | The Skeletonisation method to use, possible options are `zhang`, `lee`, `thin` (from [Scikit-image Morphology module](https://scikit-image.org/docs/stable/api/skimage.morphology.html)) or the original bespoke TopoStats (height-biasing) method topostats. |
...
| `disordered_tracing` <br> ∟ `pruning_params` | `max_length` | float | `-1` | The length in nanometres below which to prune branches. Default is `-1`, meaning 15% of the total length. |
...
| | `only_height_prune_endpoints` | boolean | `True` | Whether to restrict height-based pruning to skeleton segments containing an endpoint or not. Options are; `True` which will only attempt to prune branches with an endpoint, or `False` where height pruning will attempt to prune all branches according to the height-based pruning criteria - this is useful for removing false bridging points in the skeleton. |
...
| | `rolling_window_resampling` | bool | `true` | Whether to resample the trace or not. |
| | `rolling_window_resample_regular_spatial_interval` | `float` | `0.5e-9` | The spatial interval to resample the trace to in nm. |
| | `spline_step_size` | int | `7.0e-9` | The sampling length of the spline (in meters) to obtain an average of splines. |
...
| | `image_set` | list(strings) | `[core]` | A list of which images to plot. Single options : `[all]`, `[core]`, multiples of: `filters`, `grains`, `grain_crops`, `disordered_tracing`, `nodestats`, `ordered_tracing`, and `splining`. |
...
Python library [Matplotlib](https://matplotlib.org/stable/). A custom
