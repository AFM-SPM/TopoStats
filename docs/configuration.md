...
| `log_level`  |                                | string     | `info`         | Level of verbosity to use when logging, options are in order of verbosity `debug` `info` (default), `warning`, `error`. For minimal output set to `warning` or `error`.                                                                                          |
