...
|              | `pixel_interpolation`          | string or None  | `null`    | Interpolation method for matplotlib image plots (when one or both of `axes` or `colorbar` is true). `null` is selected by default as this prevents any interpolation of underlying data which may cause unwanted artifacts or misrepresent the true image without users understanding what is occuring to their data. See [matplotlib imshow intepolations](https://matplotlib.org/stable/gallery/images_contours_and_fields/interpolation_methods.html) for details on alternative options.          |
