...
|                 | `grain_height_removal_thresholds_std_dev` | dictionary | `[null, null], [null, null]` | Thresholds that can be used to remove detected grains from the grain mask based on their median height. These thresholds are multiplied by the standard deviation of the image to produce the true thresholds. Grains whose median falls outside the range defined by the minimum and maximum thresholds, are removed|
