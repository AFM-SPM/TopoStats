...
|                 | `cores`                     | int        | 1                           | Number of cores to use for tracing. Currently this should always be 1 if processing multiple images, in the future when processing is modularised and tracing can be run as a process on its own parallelisation can then be leveraged.                                                                                           |
