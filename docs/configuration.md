...
|                 | `pixel_interpolation` | string | `none` | Interpolation method for matplotlib image plots (when one or both of `axes` or `colorbar` is true). The recommended default of `'none'`  prevents banding that occurs in high resolution images when `nearest` is used. Interpolation should be used with caution as it may misrepresent the true image data. If interpolation is required it is suggested to use `gaussian`. For all possible options refer to the [matplotlib imshow interpolations documentation](https://matplotlib.org/stable/gallery/images_contours_and_fields/interpolation_methods.html). |
