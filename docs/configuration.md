...
| `nodestats`     | `run`                             | boolean        | `true`                      | Whether to run the Disordered Traces pipeline. Options : true, false                                                                                                                                                                                                                                                                   |
|                 | `node_joining_length`             | int            | `7.0e-9`                    | The length over which to join nearby crossing regions in the skeleton. A good estimate is ~2 times the width of your filament/strand.                                                                                                                                                                                                  |
|                 | `node_extend_dist`                | str            | `14.0e-9`                   | The distance over which to join odd-branched crossing points. As the algorithm pairs branches eminating from a crossing, these odd-branched crossing regions must be handled, and this connects them to form and even crossing.                                                                                                        |
|                 | `branch_pairing_length`           | str            | `20.0e-9`                   | Padding for The length out along a branch from the crossing point to pair eminating crossing branches and trace along to obtain full-width half-maximums (FWHMs) and calculate the over/underlying strands.                                                                                                                            |
