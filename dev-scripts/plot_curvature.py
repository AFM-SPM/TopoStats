...
def _(dir_base, pd, plt, sns):
    ...
    # Replace NaNs in the num_crossings column to zeros
    df_data["num_crossings"] = df_data["num_crossings"].fillna(0)
...
def _(combinations, mpl, np, pd, stats):
    ...
    def perform_t_test(
        ...
        if any(np.isnan(data_per_sample[sample_type_1])) or any(
            ...
            raise ValueError(f"There are nans in the dataset for {value_column}")
        # If either are not normal, use a non-parametric test, eg Mann-Whitney
        # Else, use a t-test
        sample_1_normal, sample_1_normality_metric = normality_test_with_result(
            data_per_sample[sample_type_1]
        )
        sample_2_normal, sample_2_normality_metric = normality_test_with_result(
            data_per_sample[sample_type_2]
        )
        if (not sample_1_normal) or (not sample_2_normal):
            if not sample_1_normal:
                print(
                    f"{value_column} {sample_type_1} not normal: {sample_1_normality_metric:.4f}"
                )
            if not sample_2_normal:
                print(
                    f"{value_column} {sample_type_2} not normal: {sample_2_normality_metric:.4f}"
                )
    ...
    def add_significance_to_current_plot(
        ...
        for i, (sample_a, sample_b, test_name, p) in enumerate(test_results):
            print(
                f"pairwise test: {value_column} : [{sample_a:>15}, {sample_b:>15}]: {test_name:>20} : {p:.3e}"
            )
...
def _(
    ...
    metrics = [
        ...
        ("curvature_grain_num_turns", "Number of turns"),
    ]
        ...
        for group in groups:
            ...
            for parameter_name, plot_title in metrics:
                for species in group:
                    ...
                    print(
                        f"median {parameter_name} for {species}: {np.median(data_species_parameter)}"
                    )
                ...
                ax.tick_params(
                    "x",
                    rotation=45,
                    labelsize=fig_axes_tick_font_size * fontsize_multiplier,
                )
                ax.tick_params(
                    "y", labelsize=fig_axes_tick_font_size * fontsize_multiplier
                )
                ax.set_ylabel(
                    plot_title,
                    fontsize=fig_axes_label_font_size * fontsize_multiplier,
                )
                ax.set_xlabel("")
                ax.spines["left"].set_linewidth(axes_linewidth)
                ax.spines["bottom"].set_linewidth(axes_linewidth)
...
def _(LoadScans, PLOTTINGARGS, Path, dir_base, dir_output_plots, mpl, np, plt):
    ...
    sc_file = dir_base / "sc" / "processed" / "topostats" / "20251031_SCpicoz_8ng_nicl.0_00003.topostats"
    nicked_file = dir_base / "nicked" / "processed" / "topostats" / "20251031_nicked_picoz_8ng_nicl.0_00062.topostats"
    at_file = dir_base / "3at" / "processed" / "topostats" / "20251107_8ng_3ATpicoz_nicl.0_00039.topostats"
    tel12_file = dir_base / "20260310_TC_picoztel12" / "processed" / "topostats" / "20260310_TC_8ngpicoz_TEL12_nicl.0_00026.topostats"
    ...
    def plot_curvature(
        ...
        loadscans.get_data()
        topo_instance = loadscans.img_dict[list(loadscans.img_dict.keys())[0]]
        pixel_to_nm_scaling = topo_instance.pixel_to_nm_scaling
        grain_crop = topo_instance.grain_crops[grain_number]
        curvatures_mol_0 = np.abs(grain_crop.ordered_trace.molecule_data[0].curvature_stats.curvatures)
        image = topo_instance.image
        mol_0_bbox = grain_crop.ordered_trace.molecule_data[0].bbox
        # pad the bbox
        ...
        splined_points_mol_0 = grain_crop.ordered_trace.molecule_data[0].splined_coords
