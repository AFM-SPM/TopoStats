...
class dnaTrace:
    ...
    def re_add_holes(self, orig_mask, new_mask, holearea_min_max=None):
        ...
        """
        if holearea_min_max is None:
            holearea_min_max = [4, 100]
    ...
    def linear_or_circular(self, traces: dict):
        ...
        This function is sensitive to branches from the skeleton so might need to
        implement a function to remove them
        ...
        for dna_num in sorted(self.ordered_traces.keys()):
            ...
            for coord_num, trace_coordinate in enumerate(individual_skeleton):
                ...
                if perp_direction == 'negative diaganol':
                    ...
                    quit('A fatal error occured in the CorrectHeightPositions function, this was likely
                    caused by miscalculating vector angles')
    ...
    def get_splined_traces(self):
        ...
        # FIXME : Iterate over self.fitted_traces directly use either
        # self.fitted_traces.values() or self.fitted_trace.items()
        for dna_num in sorted(self.fitted_traces.keys()):
            ...
            if self.mol_is_circular[dna_num]:
                ...
                # if nbr/step_size > 4: #the degree of spline fit is 3 so
                # there cannot be less than 3 points in splined trace
                ...
                for i in range(step_size):
                    ...
                    try:
                        tck, _ = interp.splprep([x_sampled, y_sampled], s=0, per=2, quiet=1, k=3)
                        ...
                        try:
                            tck, _ = interp.splprep([x, y], s=0, per=2, quiet=1)
                            out = interp.splev(np.linspace(0, 1, nbr * step_size), tck)
                            splined_trace = np.column_stack((out[0], out[1]))
                        except ValueError:  # sometimes even the ordered_traces
                            # are too bugged out so just delete these traces
                            self.mol_is_circular.pop(dna_num)
                            self.disordered_traces.pop(dna_num)
                            self.grains.pop(dna_num)
                            self.ordered_traces.pop(dna_num)
    ...
    def find_curvature(self):
        ...
        for dna_num in sorted(self.splined_traces.keys()):  # the number of molecules identified
            ...
            for i, (_, _) in enumerate(self.splined_traces[dna_num]):
    ...
    def saveCurvature(self):
        ...
        for dna_num in sorted(self.curvature.keys()):
            for i, [_, contour, c] in enumerate(self.curvature[dna_num]):
                try:
    ...
    def measure_contour_length(self):
        ...
        for dna_num in sorted(self.splined_traces.keys()):
            ...
            if self.mol_is_circular[dna_num]:
                for num, _ in enumerate(self.splined_traces[dna_num]):
    ...
    def writeCoordinates(self, dna_num):
        ...
        savename = os.path.join(directory, os.path.basename(self.filename)[:-4])
        for _, (x, y) in enumerate(self.splined_traces[dna_num]):
            try:
