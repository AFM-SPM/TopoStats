...
class pruneSkeleton:
    ...
    @staticmethod
    def _prune_max(image: npt.NDArray, skeleton: npt.NDArray, prune_args: dict) -> npt.NDArray:
...
class topostatsPrune:
    ...
    def _prune_single_skeleton(self, single_skeleton: npt.NDArray, max_length=-1) -> npt.NDArray:
...
def remove_bridges_abs_mean(skeleton, image, threshold) -> npt.NDArray:
    """Identifies branches which cross the skeleton in places they shouldn't due to
...
def remove_bridges_mid(skeleton, image, threshold) -> npt.NDArray:
    """Identifies branches which cross the skeleton in places they shouldn't due to
    poor thresholding and holes in the mask. Segments are removed based on their minium
    heights being lower then the mean-threshold.
    I.e. for dna a thresold of 0.85nm would remove all segments whose lowest point is <
